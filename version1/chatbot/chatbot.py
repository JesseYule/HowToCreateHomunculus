...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        # parser.add_argument() 添加参数,模型需要用到这些参数
    ...
    def main(self, args=None):
        ...
        # (but need to be called before_getSummaryName)
        # 读取模型位置、global step之类的模型参数
        ...
        # Saver/summarie，saver保存文件，summary用来保存图，方便tensorboard查看
        ...
        # testmode有三种模式：all、interact、daemon，all就是指测试全部训练集看看模型的效果
        ...
        try:  # If the user exit while training, we still try to save the model
            for e in range(self.args.numEpochs):
                ...
                for nextBatch in tqdm(batches, desc="Training"):
                    ...
                    # 后面只是把训练得到的信息拿出来，保存起来，或者在终端展示一下
                    assert len(ops) == 2  # training, loss

                    # ops，feedDict = self.optOp, self.lossFct，feedDict
                    ...
                    if self.globStep % 100 == 0:
                        perplexity = math.exp(float(loss)) if loss < 300 else float("inf")  # 困惑度作为损失函数
    ...
    def loadModelParams(self):
        """ Load some values associated with the current model, like the current globStep value
